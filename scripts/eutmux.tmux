...
init() {
  # check tmux version which must be 3.2+
  local min_version="3.2"
  local version=$(tmux -V | grep -Eo '[[:digit:]]+.[[:digit:]]+')
  if [ $(echo "${version} < ${min_version}" | bc) -eq 1 ]; then
    _warn "Tmux version ${version} is too old. It must be ${min_version} or above"
    exit ${EXIT_ABNORMAL}
  fi
...
setup() {
  ...
  DYNAMIC_PALETTE_FILENAME="eutmux${PALETTE_FILE_EXTENSION}"
...
replace_legacy_placeholders() {
  index=0
  base_color_index=0
  for placeholder in "${PLACE_HOLDERS[@]}"; do
...
while getopts "abBc:dDfhLp:rsRt:T:" opt; do
  ...
  d)
    CREATE_DYNMIC_THEME=${TRUE}
    FORCE_SAVE_THEME=${TRUE}
    ;;
  ...
  T)
    NEW_THEME_NAME="$OPTARG"
    save_dynamic_theme "${NEW_THEME_NAME}"
    exit
    ;;
  * | h | ?)
    usage

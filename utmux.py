...
class Constructor:
    ...
    def __init__(self, utmux: dict, theme: Theme):
        """Constructor."""
        self.general = utmux.get("general")
        self.terminal = utmux.get("terminal", theme.get("terminal"))
        self.status_line = utmux.get("status_line", theme.get("status_line"))
        self.foreground = self.status_line.get("foreground")
        self.background = self.status_line.get("background")
        self.status_left = utmux.get("status_left")
        self.window = utmux.get("window")
        self.status_right = utmux.get("status_right")
        self.theme = theme
...
def utmux(config_file="utmux.yaml"):
    ...
    utmux_config_home = f"{xdg_config_home}/utmux.tmux"
    _config_file = f"{utmux_config_home}/{config_file}"
    ...
    utmux_workdir = os.getenv("UTMUX_WORKDIR")
    os.chdir(utmux_workdir)
    with open(dynamic_config_file_name, "r", encoding=UTF_8) as config:
        utmux = yaml.load(config, Loader=Loader)

    # if specified theme doesn't have corresponding file, then fall-back to
    # the default theme - utmux theme.
    theme_name = utmux.get("theme", "utmux")
    ...
    # utmux theme, otherwise, load the theme file from UTMUX_CONFIG_HOME
    if not os.path.exists(theme_filename):
        if os.path.exists(f"{utmux_config_home}/{theme_filename}"):
            theme_filename = f"{utmux_config_home}/{theme_filename}"
        else:
            theme_filename = "utmux.theme.yaml"
    ...
    with open(theme_filename, "r", encoding=UTF_8) as theme_file:
        ...
        constructor = Constructor(utmux, theme)
